...
<section class="ind_top">
    ...
    <div class="width85">
        <h4 class="topic mt-5">
            {{$latest_news_data->topic}}
        </h4>
        <h3 class="common_caption width60 ">
            {{$latest_news_data->title}}
        </h3>
        <span class="common_date alter_date">{{$latest_news_data->created_at}}</span>
        <div class="pr_main_flex d-flex justify-content-between">
            <div class="pr_main_item1">

                <div class="pr_main_item1_img" style='background-image:url("{{ asset("assets/images/$latest_news_data->image") }}")'>
                </div>
                <p class='common_text'>
                    {{$latest_news_data->description}}
                </p>
            </div>
            <div class="pr_main_item2">
                <h4>Last press releases</h4>
                @foreach($latest_news_last_tree as $el_pr3)
                <a class='a_no_dec' href="{{route('our_news_separate', ['locale' => app()->getLocale(), 'id' => $el_pr3->id] )}}">
...
<section class="topics_sec width85 mt-5 mb-5">
    ...
    <div class="tab-content" id="pills-tabContent">
        @foreach($topics as $el_top)
        @if($el_top->topic == $latest_news_data->topic)
        <div class="tab-pane fade show active" id="pills-topic<?php echo $jk;
            ...
            <div class='tab_content_flex d-flex justify-content-around'>
                @foreach($latest_news_rest as $el_rest)
                @if($latest_news_data->topic==$el_rest->topic)
                <div class='tab_content_item'>
                    <a class='a_no_dec' href="{{route('our_news_separate', ['locale' => app()->getLocale(), 'id' => $el_rest->id] )}}">
