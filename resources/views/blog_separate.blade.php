    ...
    <div class="width85">
        ...
        <div class="pr_main_flex d-flex justify-content-between">
            <div class="pr_main_item1">
                ...
                <div class="pr_main_item1_img" style='background-image:url("{{ asset("assets/images/$data->image") }}")'>
                </div>
                <p class='common_text'>
                    {{$data->description_en}}
                </p>
            ...
            <div class="pr_main_item2">
                <h4>More Recent</h4>
                @foreach($BlogLastTree as $el_pr3)
                <a class='a_no_dec' href="{{route('blog_separate', ['locale' => app()->getLocale(), 'id' => $el_pr3->id] )}}">
                    <div class='pr_last3_content'>
                        <div class="pr_last3_img" style='background-image:url("{{ asset("assets/images/$el_pr3->image") }}")'>
                        </div>
                        <h5>{{$el_pr3->title_en}}</h5>
                    </div>
                </a>
                @endforeach
            </div>
...
<section class="topics_sec width85 mt-5 mb-5">
    ...
    <div class='topics_display d-flex'>
        ...
        <div>
            ...
            @foreach($BlogTopics as $el_top)
                <li class="nav-item nav-pr_release">
                    <a class="nav-link" id="pills-contact-tab" data-toggle="pill" href="#pills-topic<?php echo $j; $j++;?> " role="tab" aria-controls="pills-contact" aria-selected="false">
                        <span class='topic_span'>{{$el_top->topic}}</span>
                    </a>
                </li>
            @endforeach
            </ul>
    ...
    @foreach($BlogTopics as $el_top)
        @if($el_top->topic == $data->category_en)
