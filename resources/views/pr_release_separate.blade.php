...
<section class="ind_top">
    ...
    <div class="width85">
        <h4 class="topic mt-5">
            {{$pr_release_data->topic}}
        </h4>
        <h3 class="common_caption width60 ">
            {{$pr_release_data->title}}
        </h3>
        <span class="common_date alter_date">{{$pr_release_data->created_at}}</span>
        <div class="pr_main_flex d-flex justify-content-between">
            <div class="pr_main_item1">

                <div class="pr_main_item1_img" style='background-image:url("{{ asset("assets/images/$pr_release_data->image") }}")'>
                </div>
                <p class='common_text'>
                    {{$pr_release_data->description}}
                </p>
            </div>
            <div class="pr_main_item2">
                <h4>Last press releases</h4>
                @foreach($pr_last_tree as $el_pr3)
                <a class='a_no_dec' href="{{route('pr_separate', ['locale' => app()->getLocale(), 'id' => $el_pr3->id] )}}">
...
<section class="topics_sec width85 mt-5 mb-5">
    ...
    <div class="tab-content" id="pills-tabContent">
        @foreach($topics as $el_top)
        @if($el_top->topic == $pr_release_data->topic)
        <div class="tab-pane fade show active" id="pills-topic<?php echo $jk;
            ...
            <div class='tab_content_flex d-flex justify-content-around'>
                @foreach($pr_release_rest as $el_rest)
                @if($pr_release_data->topic==$el_rest->topic)
